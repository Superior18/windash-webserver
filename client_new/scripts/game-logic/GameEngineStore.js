define([
    ...
    function Engine() {
         ...
         * e.g: { user1: { bet: duffs, stopped_at: 200 }, user2: { bet: duffs } }
         */
        ...
        self.ws.on('game_started', function(bets) {
            ...
            Object.keys(bets).forEach(function(username) {
                if (self.username === username)
                    self.balanceDuffs -= bets[username];
         ...
         * @param {object} data.bonuses - List of bonuses of each user, in duffs
        ...
        self.ws.on('game_crash', function(data) {
            ...
            for (var user in data.bonuses) {
                ...
                if (self.username === user) {
                    self.balanceDuffs += data.bonuses[user];
                }
            }
         ...
         * @param {number} resp.bet - The player bet in duffs
         */
        ...
        self.ws.on('cashed_out', function(resp) {
            ...
            if (self.username === resp.username) {
                self.cashingOut = false;
                self.balanceDuffs += self.playerInfo[resp.username].bet * resp.stopped_at / 100;
            }
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: window.DEV_OTT? window.DEV_OTT : ott },
                    function(err, resp) {
                        ...
                        self.balanceDuffs = resp.balance_duffs;
     ...
     * @param {number} amount - Bet amount in μDash
    ...
    Engine.prototype.calcBonuses = function() {
        ...
        //{ user1: { bet: duffs, stopped_at: 200 }, user2: { bet: duffs } } -> [ user1: { bet: duffs, stopped_at: 200 } ... ]
