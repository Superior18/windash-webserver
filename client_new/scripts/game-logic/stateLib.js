...
define([
    ...
    return {
        ...
        /** Parse the bet string in μDash and returns a integer **/
        ...
        canUserBet: function(balanceDuffs, betStringBits, betInvalid, autoCashOutInvalid) {
            var betAmountDuffs = this.parseBet(betStringBits);

            if(balanceDuffs < 100)
                return new Error('Not enough μDash to play');
            if(betInvalid)
                return new Error(betInvalid);
            if(autoCashOutInvalid)
                return new Error(autoCashOutInvalid);
            if(balanceDuffs < betAmountDuffs)
                return new Error('Not enough μDash');
