...
exports.security = function(req, res) {
    ...
    if (!user.mfa_secret) {
        ...
        var qrUri = 'otpauth://totp/WinDash:' + user.username + '?secret=' + user.mfa_potential_secret + '&issuer=WinDash';
...
exports.enableMfa = function(req, res, next) {
    ...
    if (otp !== expected) {
        ...
        var qrUri = 'otpauth://totp/WinDash:' + user.username + '?secret=' + secret + '&issuer=WinDash';
        user.qr_svg = qr.imageSync(qrUri, {type: 'svg'});
        user.sig = sig;
...
exports.transferRequest = function(req, res) {
 ...
 exports.handleTransferRequest = function (req,res,next){
    ...
    amount = Math.round(parseFloat(amount) * 100);
     ...
     if (amount < 10000)
        return res.render('transfer-request', { user: user, id: uuid.v4(),  warning: 'Must transfer at least 100 μDash' });
...
exports.handleWithdrawRequest = function(req, res, next) {
    ...
    try {
        var version = WAValidator.validate(destination, 'DASH');
        if (!version)
            return res.render('withdraw-request', { user: user,  id: uuid.v4(), warning: 'Destination address is not a dash one' });
    } catch(ex) {
