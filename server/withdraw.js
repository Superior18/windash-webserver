...
// Doesn't validate
module.exports = function(userId, duffs, withdrawalAddress, withdrawalId, callback) {
    var minWithdraw = config.MINING_FEE + 100;
    assert(typeof userId === 'number');
    assert(duffs >= minWithdraw);
    assert(typeof withdrawalAddress === 'string');
    assert(typeof callback === 'function');

    db.makeWithdrawal(userId, duffs, withdrawalAddress, withdrawalId, function (err, fundingId) {
        if (err) {
            if (err.code === '23514')
        ...
        var amountToSend = (duffs - config.MINING_FEE) / 1e8;
