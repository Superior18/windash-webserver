...
exports.getUserFromUsername = function(username, callback) {
    ...
    query('SELECT * FROM users_view WHERE lower(username) = lower($1)', [username], function(err, data) {
        ...
        assert(data.rows.length === 1);
        var user = data.rows[0];
        assert(typeof user.balance_duffs === 'number');
...
function addDuffs(client, userId, amount, callback) {

    client.query('UPDATE users SET balance_duffs = balance_duffs + $1 WHERE id = $2', [amount, userId], function(err, res) {
        if (err) return callback(err);
        assert(res.rowCount === 1);
        callback(null);
    });
        ...
        var tasks = userNames.map(function(username) {
            return function(callback) {
                ...
                client.query('SELECT id FROM users WHERE lower(username) = lower($1)', [username], function(err, result) {
                    if (err) return callback('unable to add μDash');
...
exports.makeTransfer = function(uid, fromUserId, toUsername, duffs, callback){
    assert(typeof fromUserId === 'number');
    assert(typeof toUsername === 'string');
    assert(typeof duffs === 'number');

    // Update balances
    getClient(function(client, callback) {
        async.waterfall([
            function(callback) {
                client.query("UPDATE users SET balance_duffs = balance_duffs - $1 WHERE id = $2",
                  [duffs, fromUserId], callback)
            },
            function(prevData, callback) {
                client.query(
                  "UPDATE users SET balance_duffs = balance_duffs + $1 WHERE lower(username) = lower($2) RETURNING id",
                  [duffs, toUsername], function(err, data) {
                      if (err)
                          return callback(err);
                      if (data.rowCount === 0)
            ...
            function (toUserId, callback) {
                client.query(
                  ...
                  [uid, fromUserId, toUserId, duffs], callback);
            }
        ], function(err) {
            if (err) {
...
exports.makeWithdrawal = function(userId, duffs, withdrawalAddress, withdrawalId, callback) {
    ...
    getClient(function(client, callback) {
        ...
        client.query("UPDATE users SET balance_duffs = balance_duffs - $1 WHERE id = $2",
            [duffs, userId], function(err, response) {
            if (err) return callback(err);
            ...
            client.query('INSERT INTO fundings(user_id, amount, bitcoin_withdrawal_address, withdrawal_id) ' +
                ...
                [userId, -1 * duffs, withdrawalAddress, withdrawalId],
