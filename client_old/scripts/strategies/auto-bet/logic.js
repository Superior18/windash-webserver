define(['lib/clib'], function(Clib){
    ...
    return function (settings) {
         ...
         *  @param {number} settings.baseBet - Base bet in μDash
        ...
        return function(engine) {
            //MoneyBot\n\
            var baseBetDuffs = settings.baseBet * 100;
            var currentBet = baseBetDuffs;
            ...
            engine.on('game_starting', function() {
                ...
                if(fixedCurrentBet > 0 && fixedCurrentBet <= engine.getBalance() && fixedCurrentBet <= engine.getMaxBet() && fixedCurrentBet <= maxBetStop) {
                    ...
                    engine.stop();
                    console.log('You ran out of μDash or exceeded the max bet or betting nothing :(');
                }
            });
        }
